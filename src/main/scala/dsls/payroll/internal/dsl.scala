package dsls.payroll.internal

import scala.language.postfixOps
import dsls.payroll.common._
...
object dsl {
  def biweekly(f: DeductionsBuilder => Deductions) = f(new DeductionsBuilder("Biweekly", 26.0))
  ...
  class DeductionsBuilder(name: String,
    ...
    def insurance_premiums(amount: Amount): DeductionsBuilder = {
      deductions = deductions :+ Deduction("insurance_premiums", amount)
      this
    }

    def retirement_savings(amount: Amount): DeductionsBuilder = {
      deductions = deductions :+ Deduction("retirement_savings", amount)
      this
    }
  }
}
